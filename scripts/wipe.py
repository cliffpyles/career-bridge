"""
Wipe the development database: truncate all user data without re-seeding.
...
async def _run() -> None:
    ...
    async with factory() as session:
        ...
        # saved_jobs must come before jobs (FK: saved_jobs.job_id → jobs.id).
...
def main() -> None:
    ...
    try:
        ...
        print(f"\nWipe failed: {exc}", file=sys.stderr)
        sys.exit(1)
